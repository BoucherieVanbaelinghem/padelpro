...
// Marqueur de version bien visible : permet de vérifier depuis la console
// que le navigateur exécute bien CETTE version corrigée du fichier, et pas
// une ancienne copie restée en mémoire (onglet restauré par la session
// précédente d'Edge, cache, etc.). S'il n'apparaît pas dans la console au
// chargement, le fichier réellement exécuté n'est pas celui-ci.
console.log('%cPadelPro storage.js — build 2026-07-03-fix3 (garde-fou anti-boucle actif)', 'color:#16a34a;font-weight:bold');
...
const Storage = (() => {
  ...
  //
  // ATTENTION : ne PAS créer un nouveau tournoi au moindre échec de
  // recherche de _activeId dans le cache. Les listeners Firestore
  // (tournaments / players / meta) sont indépendants et peuvent livrer
  // leurs données dans le désordre ou avec un léger retard (démarrage,
  // reconnexion réseau, filet de sécurité 8s de ready() si hors ligne).
  // Dans ces cas transitoires, _activeId pointe vers un tournoi qui EXISTE
  // bel et bien côté serveur, simplement pas encore reçu localement — ce
  // n'est pas une preuve qu'il faille en fabriquer un nouveau.
  //
  // L'ancien comportement (créer + sauvegarder immédiatement dès que
  // getTournament(_activeId) échouait) provoquait la création silencieuse
  // de tournois vides à chaque petite désynchronisation, et chaque création
  // écrivait dans Firestore, ce qui redéclenchait un onSnapshot, donc un
  // nouvel appel à getActive()... d'où des tournois vides qui
  // s'accumulaient tout seuls et un écran qui clignotait (re-rendu à
  // chaque cycle).
  const getActive = () => {
    if (_activeId) {
      ...
      // _activeId introuvable dans le cache : si d'autres tournois existent
      // déjà, on bascule dessus (le plus récent) plutôt que d'en créer un
      // vide — on laisse le prochain onRemoteChange corriger l'état une
      // fois les vraies données synchronisées.
      const existing = Object.values(_tournamentsCache)
        .sort((a, b) => (b.updatedAt || '').localeCompare(a.updatedAt || ''));
      if (existing.length > 0) return existing[0];

      // Cache pas encore chargé du tout (tournaments/meta) : renvoyer un
      // tournoi vide non persisté en mémoire, le temps que les vraies
      // données arrivent, plutôt que d'écrire un nouveau tournoi fantôme.
      if (!_initFlags.tournaments || !_initFlags.meta) return _emptyTournament();

      // Sinon (cache bien chargé, vraiment aucun tournoi nulle part — ex :
      // le dernier tournoi actif vient d'être supprimé) : création légitime.
    }
  ...
  // ── Nouveau tournoi ────────────────────────────────────────────
  //
  // Garde-fou anti-boucle : indépendamment de tout ce qui peut appeler
  // createNewTournament() (getActive() en secours, boutons "Nouveau
  // tournoi", onglet resté ouvert avec une ancienne version du code en
  // mémoire, plusieurs onglets ouverts en même temps...), on refuse
  // physiquement d'écrire un nouveau tournoi en base si un autre vient
  // d'être créé il y a moins de 4 secondes. C'est la dernière ligne de
  // défense contre les créations en rafale (des centaines/milliers de
  // tournois vides) déjà observées, quelle qu'en soit la cause exacte.
  let _lastAutoCreateAt = 0;
  const _MIN_CREATE_INTERVAL_MS = 4000;

  const createNewTournament = () => {
    const now = Date.now();
    if (now - _lastAutoCreateAt < _MIN_CREATE_INTERVAL_MS) {
      console.error(
        'PadelPro — création de tournoi bloquée par le garde-fou anti-boucle ' +
        '(une autre création a eu lieu il y a moins de 4s). Si vous n\'avez pas ' +
        'cliqué "Nouveau tournoi" vous-même juste avant, un bug tente de créer des ' +
        'tournois en boucle : fermez TOUS les onglets PadelPro ouverts (vérifiez ' +
        'qu\'il n\'y en a pas un autre oublié en arrière-plan), puis rouvrez-en un seul.'
      );
      const fallback = Object.values(_tournamentsCache)
        .sort((a, b) => (b.updatedAt || '').localeCompare(a.updatedAt || ''))[0];
      if (fallback) return fallback;
    }
    _lastAutoCreateAt = now;
